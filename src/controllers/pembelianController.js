...
class PembelianController {
    ...
    try {
        ...
        for (const product of products) {
            ...
            pembelianDetails.push({
                product_id,
                qty,
                harga_beli: productData.harga_beli, // Pastikan harga beli disertakan
        ...
        for (const detail of pembelianDetails) {
            await PembelianDetail.create({
                ...
                qty: detail.qty,
                harga_beli: detail.harga_beli, // Pastikan harga beli disertakan
            ...
            // Perbarui stok dan harga beli jika berbeda
            await tokoProduct.increment("stok", { by: detail.qty });

            // Jika harga beli lebih murah dari yang ada di toko, update
  ...
  getWeeklyExpenses = async (req, res) => {
    try {
      ...
      const timeZoneOffset = 7 * 60; // WIB offset in minutes
      ...
      const weeklyExpenses = await Pembelian.findAll({
        attributes: [
          [Sequelize.literal('DATE(CONVERT_TZ(tanggal_pembelian, "+00:00", "+07:00"))'), "tanggal"],
        ...
        group: [Sequelize.literal('DATE(CONVERT_TZ(tanggal_pembelian, "+00:00", "+07:00"))')],
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        date.setDate(endOfDay.getDate() - i);
        
        const currentDateStr = new Date(date.getTime() - (timeZoneOffset * 60000))
          .toISOString()
          .split('T')[0];
        ...
        completeWeekData.push({
          ...
          total_pengeluaran: existingData ? 
            parseFloat(existingData.getDataValue("total_pengeluaran")) || 0 : 0
        });
      }
